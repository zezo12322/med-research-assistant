    ...
    try {
      ...
      for (var entry in entries) {
        ...
        for (var field in fields) {
          final fieldValue = entry.fieldValues.firstWhere(
            (fv) => fv.fieldId == field.fieldId,
            orElse: () => throw Exception('Field value not found'),
          );
          
          // Handle different field types
          String value = fieldValue.value;
          if (field.type.toString().contains('date')) {
            try {
              final date = DateTime.parse(value);
              value = date.toString().split(' ')[0]; // Date only
            } catch (_) {}
          } else if (field.type.toString().contains('image')) {
